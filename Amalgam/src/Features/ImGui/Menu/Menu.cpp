...
void CMenu::DrawMenu()
{
	...
	if (Begin("Main", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground))
	{
		...
		GetForegroundDrawList()->AddRect(vWindowPos, vWindowPos + vWindowSize, ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }));
		GetForegroundDrawList()->AddRect(vWindowPos + ImVec2(1, 1), vWindowPos + vWindowSize - ImVec2(1, 1), ImGui::GetColorU32({ 0.207f, 0.207f, 0.207f, 1.f }));
		GetForegroundDrawList()->AddRect(vWindowPos + ImVec2(2, 2), vWindowPos + vWindowSize - ImVec2(2, 2), ImGui::GetColorU32({ 0.109f, 0.109f, 0.109f, 1.f }));
		GetForegroundDrawList()->AddRect(vWindowPos + ImVec2(3, 3), vWindowPos + vWindowSize - ImVec2(3, 3), ImGui::GetColorU32({ 0.109f, 0.109f, 0.109f, 1.f }));
		GetForegroundDrawList()->AddRect(vWindowPos + ImVec2(4, 4), vWindowPos + vWindowSize - ImVec2(4, 4), ImGui::GetColorU32({ 0.207f, 0.207f, 0.207f, 1.f }));
		GetForegroundDrawList()->AddRect(vWindowPos + ImVec2(5, 5), vWindowPos + vWindowSize - ImVec2(5, 5), ImGui::GetColorU32({ 0.f, 0.f, 0.f, 1.f }));
		GetForegroundDrawList()->AddRectFilledMultiColor(vWindowPos + ImVec2(5, 5), vWindowPos + ImVec2(vWindowSize.x, 6) - ImVec2(5, 0), ImGui::GetColorU32({ 1, 0, 0, 1 }), ImGui::GetColorU32({ 1, 1, 1, 1 }), ImGui::GetColorU32({ 1, 1, 1, 1 }), ImGui::GetColorU32({ 1, 0, 0, 1 }));
		PopClipRect();
			...
			PushDisabled(F::PlayerUtils.m_bLoad);
			{
				...
				if (FButton("Export", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					...
					if (fStream.is_open())
					{
						...
						SDK::Output("SemataryHook", "Copied playerlist to clipboard", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
				}

				{
					...
					if (FButton("Import", FButtonEnum::Fit | FButtonEnum::SameLine))
					{
						...
						{
							SDK::Output("SemataryHook", "Failed to import playerlist", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
						}
					}
					...
					if (FBeginPopupModal("ImportPlayerlist", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
					{
						...
						if (FButton("Import", FButtonEnum::Left))
						{
							...
							F::PlayerUtils.m_bSave = true;
							SDK::Output("SemataryHook", "Imported playerlist", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
				...
				if (FButton("Backup", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					try
					{
						...
						SDK::Output("SemataryHook", "Saved backup playerlist", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
					catch (...)
					{
						SDK::Output("SemataryHook", "Failed to backup playerlist", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
				}
			}
			PopDisabled();
